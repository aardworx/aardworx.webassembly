...
module internal FXAA =
    ...
    type Sampler2dExtensions private() =
        ...
        static member SampleLevelFXAA(x : Sampler2d, fragCoord : V2f, level : float32, preset : PRESET, edgeThreshold : float32, edgeThresholdMin : float32, subpix : float32) =
            ...
            if (earlyExit) then
                ...
                let subpixC = clamp 0.0f 1.0f (abs subpixB * subpixRcpRange)
    ...
    let fxaaExtreme (v : Vertex) =
        fragment {
            ...
            let inverseVP = 1.0f / V2f (diffuseSampler.GetSize 0)
            let luma = V3f(0.299f, 0.587f, 0.114f)

            //let P = FXAA_PRESET_39__P

            let mutable posM = v.tc
            let mutable lumaS = Vec.dot (diffuseSampler.SampleLevel(posM + V2f( 0.0f,  1.0f) * inverseVP, 0.0f).XYZ) luma
            let mutable lumaE = Vec.dot (diffuseSampler.SampleLevel(posM + V2f( 1.0f,  0.0f) * inverseVP, 0.0f).XYZ) luma
            let mutable lumaN = Vec.dot (diffuseSampler.SampleLevel(posM + V2f( 0.0f, -1.0f) * inverseVP, 0.0f).XYZ) luma
            let mutable lumaW = Vec.dot (diffuseSampler.SampleLevel(posM + V2f(-1.0f,  0.0f) * inverseVP, 0.0f).XYZ) luma
            let rgbM = diffuseSampler.SampleLevel(posM, 0.0f)
            let lumaM = Vec.dot rgbM.XYZ luma
            ...
            let rangeMax = max maxWN maxESM
            let rangeMin = min minWN minESM
            let rangeMaxScaled = rangeMax * 0.063f
            let range = rangeMax - rangeMin
            let rangeMaxClamped = max 0.0625f rangeMaxScaled

            if range < rangeMaxClamped then
                return rgbM
            else
                let lumaNW = Vec.dot (diffuseSampler.SampleLevel(posM + V2f(-1.0f, -1.0f) * inverseVP, 0.0f).XYZ) luma
                let lumaSE = Vec.dot (diffuseSampler.SampleLevel(posM + V2f( 1.0f,  1.0f) * inverseVP, 0.0f).XYZ) luma
                let lumaNE = Vec.dot (diffuseSampler.SampleLevel(posM + V2f( 1.0f, -1.0f) * inverseVP, 0.0f).XYZ) luma
                let lumaSW = Vec.dot (diffuseSampler.SampleLevel(posM + V2f(-1.0f,  1.0f) * inverseVP, 0.0f).XYZ) luma
                ...
                let subpixD = ((-2.0f)*subpixC) + 3.0f
                let mutable lumaEndN = Vec.dot (diffuseSampler.SampleLevel(posN, 0.0f).XYZ) luma
                let subpixE = subpixC * subpixC
                let mutable lumaEndP = Vec.dot (diffuseSampler.SampleLevel(posP, 0.0f).XYZ) luma
                ...
                for i in 2..FXAA_PRESET_39__P.Length-1 do
                    if not doneNP then brk()
                    if not doneN then lumaEndN <- Vec.dot (diffuseSampler.SampleLevel(posN, 0.0f).XYZ) luma
                    if not doneP then lumaEndP <- Vec.dot (diffuseSampler.SampleLevel(posP, 0.0f).XYZ) luma
                    if not doneN then lumaEndN <- lumaEndN - lumaNN * 0.5f
                    if not doneP then lumaEndP <- lumaEndP - lumaNN * 0.5f
                    doneN <- abs lumaEndN >= gradientScaled
                    doneP <- abs lumaEndP >= gradientScaled
                ...
                return V4f(diffuseSampler.SampleLevel(posM, 0.0f).XYZ, rgbM.W)
        }
